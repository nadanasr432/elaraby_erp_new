                                                            ...
                                                            <div class="dropdown-menu p-0">
                                                                ...
                                                                <a href="{{ route('client.sale_bills.edit', [$sale_bill->token, $sale_bill->company_id]) }}"
                                                                    ...
                                                                    <svg style="width: 15px; fill: #1956ad;display: inline;margin-left: 5px;"
                                                                        ...
                                                                        viewBox="0 0 512 512">
                                                                        <path
                                                                            d="M441 58.9L453.1 71c9.4 9.4 9.4 24.6 0 33.9L424 134.1 377.9 88 407 58.9c9.4-9.4 24.6-9.4 33.9 0zM209.8 256.2L344 121.9 390.1 168 255.8 302.2c-2.9 2.9-6.5 5-10.4 6.1l-58.5 16.7 16.7-58.5c1.1-3.9 3.2-7.5 6.1-10.4zM373.1 25L175.8 222.2c-8.7 8.7-15 19.4-18.3 31.1l-28.6 100c-2.4 8.4-.1 17.4 6.1 23.6s15.2 8.5 23.6 6.1l100-28.6c11.8-3.4 22.5-9.7 31.1-18.3L487 138.9c28.1-28.1 28.1-73.7 0-101.8L474.9 25C446.8-3 400.1-3 373.1 25z">
                                                                        </path>
                                                                    </svg>
                                                                    {{ __('sidebar.edit') }}
                                                                </a>
...
<script>
    ...
    $(document).ready(function() {
        var activateMessage = @json(__('main.activate_confirmation'));
        var deactivateMessage = @json(__('main.deactivate_confirmation'));

        $('.toggle-status').on('change', function() {
            var $checkbox = $(this);
            var saleBillId = $checkbox.data('id');
            var status = $checkbox.is(':checked') ? 'active' : 'inactive';
            var message = status === 'active' ? activateMessage : deactivateMessage;
            var confirmation = confirm(message);

            if (confirmation) {
                $.ajax({
                    url: '{{ route('client.sale_bills.toggle_status') }}',
                    method: 'POST',
                    data: {
                        _token: '{{ csrf_token() }}',
                    ...
                    success: function(response) {
                        if (response.success) {
                            const $showColumn = $checkbox.closest('tr').find('.show-column');

                            if (status === 'inactive') {
                                // Update the content for deactivated status
                                $showColumn.html(
                                    '<div class="badge badge-danger text-center">' +
                                    '@lang('main.deactivated')' +
                                    '</div>');
                            } else {
                                // Optionally, revert the content back if needed
                                // This assumes you want to reload the page; adjust if different behavior is desired
                                location.reload();
                            }

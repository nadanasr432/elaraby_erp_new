                    ...
                    <div class="card-header pb-0 control-column">
                        <div id="example1_wrapper" class="col-lg-12 dataTables_wrapper dt-bootstrap4 no-footer">
                            <div class="dt-buttons btn-group">
                                <button class="btn btn-secondary buttons-excel buttons-html5" tabindex="0"
                                    ...
                                    <a href="{{ route('groupe.export.excel') }}"
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="col-lg-12 margin-tb">
                                <a class="btn pull-left btn-primary btn-sm custom-button-size" href="{{ route('groupes.create') }}"><i
                                        class="fa fa-plus"></i> {{ __('sidebar.add-new-file_groupe') }} </a>
                            ...
                            <table class="table table-condensed table-striped table-bordered text-center table-hover"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('categories.groupe-description-ar') }}</th>
                                        <th class="text-center">{{ __('categories.groupe-description-en') }}</th>
                                ...
                                <tbody>
                                    @php
                                        $i = 0;
                                    @endphp
                                    @foreach ($categories as $groupe)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td>{{ $groupe->code }}</td>
                                            <td>{{ $groupe->description_ar }}</td>
                                            <td>{{ $groupe->description_en }}</td>
                                            <td class="control-column">
                                                <a href="{{ route('groupes.edit', $groupe->id) }}"
                                                    class="btn btn-sm btn-info" data-toggle="tooltip"
                                                    title="{{ __('main.edit') }}" data-placement="top"><i
                                                        class="fa fa-edit"></i></a>
                                                <a class="modal-effect btn btn-sm btn-danger delete_groupe"
                                                    groupe_id="{{ $groupe->id }}"
                                                    groupe_name="{{ $groupe->name }}" data-toggle="modal"
                    ...
                    <div class="modal-content modal-content-demo">
                        <div class="modal-header text-center">
                            <h6 class="modal-title w-100" style="font-family: 'Cairo'; ">
                                {{ __('groupes.delete-groupe') }}
                            </h6>
                            <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span
                                    aria-hidden="true">&times;</span></button>
                        </div>
                        <form action="{{ route('groupes.destroy', 'test') }}" method="post">
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
                            <div class="modal-body">
                                <p>{{ __('main.are-you-sure-to-delete') }}</p><br>
                                <input type="hidden" name="groupeid" id="groupeid">
                                <input class="form-control" name="groupename" id="groupename" type="text"
                                    readonly>
                            </div>

...
@section('content')
                    ...
                    <div class="card-header pb-0 control-column">
                        <div id="example1_wrapper" class="col-lg-12 dataTables_wrapper dt-bootstrap4 no-footer">
                            <div class="dt-buttons btn-group">
                                <button class="btn btn-secondary buttons-excel buttons-html5" tabindex="0"
                                    ...
                                    <a href="{{ route('building_roles.export.excel') }}"
                                    ...
                                    <div class="dropdown-menu">
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input column-checkbox" id="column1"
                                                data-column="1" data-table="categories-table" checked>
                                            <label class="form-check-label"
                                                for="column1">{{ __('categories.code') }}</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input column-checkbox" id="column2"
                                                data-column="2" data-table="categories-table" checked>
                                            <label class="form-check-label"
                                                for="column2">{{ __('categories.countrie-description-ar') }}</label>
                                        </div>
                                        <div class="form-check">
                                            <input type="checkbox" class="form-check-input column-checkbox" id="column3"
                                                data-column="3" data-table="categories-table" checked>
                                            <label class="form-check-label"
                                                for="column3">{{ __('categories.countrie-description-en') }}</label>
                                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            <div class="col-lg-12 margin-tb">
                                <a class="btn pull-left btn-primary btn-sm custom-button-size" href="{{ route('building_roles.create') }}"><i
                                        class="fa fa-plus"></i> {{ __('sidebar.add-new-file_building_role') }} </a>
                            ...
                            <table class="table table-condensed table-striped table-bordered text-center table-hover"
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{ __('categories.code') }}</th>
                                        <th class="text-center">{{ __('categories.countrie-description-ar') }}</th>
                                        <th class="text-center">{{ __('categories.countrie-description-en') }}</th>
                                ...
                                <tbody>
                                    @php
                                        $i = 0;
                                    @endphp
                                    @foreach ($categories as $countrie)
                                        <tr>
                                            <td>{{ ++$i }}</td>
                                            <td>{{ $countrie->code }}</td>
                                            <td>{{ $countrie->description_ar }}</td>
                                            <td>{{ $countrie->description_en }}</td>
                                            <td class="control-column">
                                                <a href="{{ route('building_roles.edit', $countrie->id) }}"
                                                    class="btn btn-sm btn-info" data-toggle="tooltip"
                                                    title="{{ __('main.edit') }}" data-placement="top"><i
                                                        class="fa fa-edit"></i></a>
                                                <a class="modal-effect btn btn-sm btn-danger delete_building_role"
                                                    countrie_id="{{ $countrie->id }}"
                                                    countrie_name="{{ $countrie->name }}" data-toggle="modal"
                    ...
                    <div class="modal-content modal-content-demo">
                        <div class="modal-header text-center">
                            <h6 class="modal-title w-100" style="font-family: 'Cairo'; ">
                                {{ __('building_roles.delete-countrie') }}
                            </h6>
                            <button aria-label="Close" class="close" data-dismiss="modal" type="button"><span
                                    aria-hidden="true">&times;</span></button>
                        </div>
                        <form action="{{ route('building_roles.destroy', 'test') }}" method="post">
                            {{ method_field('delete') }}
                            {{ csrf_field() }}
                            <div class="modal-body">
                                <p>{{ __('main.are-you-sure-to-delete') }}</p><br>
                                <input type="hidden" name="countrieid" id="countrieid">
                                <input class="form-control" name="countriename" id="countriename" type="text"
                                    readonly>
                            </div>
    ...
    <script>
        ...
        function printTable() {
            ...
            var table = document.getElementById('categories-table').cloneNode(true);

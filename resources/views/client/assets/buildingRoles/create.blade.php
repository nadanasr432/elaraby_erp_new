 ...
 @if (count($errors) > 0)
    ...
    <div class="row margin-tb">
        <div class="col-md-12 pl-4 pr-4">
            <a class="btn pull-left btn-primary btn-sm custom-font-size" href="{{ route('building_roles.create') }}">
                <i class="fa fa-plus"></i>
                @lang('main.create_country')
            </a>
            <h3 class="pull-right alert alert-sm alert-success custom-font-size">
                @lang('main.file_country')
            </h3>
        </div>
    </div>
    <div class="col-md-7 mt-5">
        <form action="{{ route('building_roles.store') }}" method="POST">
            @csrf

                        ...
                        <table class="table table-striped table-bordered zero-configuration" id="example-table"
                            ...
                            <thead>
                                <tr>
                                    ...
                                    <th class="wd-20p border-bottom-0 text-center">{{ __('main.containers_no') }}</th>
                                    <th class="wd-15p border-bottom-0 text-center">{{ __('main.weight') }}</th>
                                    <th class="wd-15p border-bottom-0 text-center">{{ __('main.height') }}</th>
                            ...
                            <tbody>
                                ...
                                @foreach ($shipments as $key => $shipment)
                                    <tr>
                                        <td>{{ ++$i }}</td>
                                        <td>{{ $shipment->payload_type }}</td>
                                        <td>{{ $shipment->containers_no }}</td>
                                        <td>{{ $shipment->weight }}</td>
                                        <td>{{ $shipment->height }}</td>
                                        <td>
                                            <a href="{{ route('shipments.edit', $shipment->id) }}" class=" "
                                            ...
                                            <a class="modal-effect delete_shipment" data-id="{{ $shipment->id }}"
                                                data-name="{{ $shipment->country }}" data-toggle="modal" href="#modaldemo9"
            ...
            <div class="modal-dialog modal-dialog-centered" supplier="document">
                <div class="modal-content modal-content-demo">
                    <div class="modal-header text-center">
                        <h6 class="modal-title w-100" style="font-family: 'Cairo'; ">@lang('main.delete_shipment')  </h6>
...
<script>
    $(document).ready(function() {
        $('.delete_shipment').click(function() {
            ...
            let actionUrl = "{{ route('shipments.destroy', ':id') }}".replace(':id',
            shipmentId);

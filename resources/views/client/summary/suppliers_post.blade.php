        ...
        <tbody>
            <tr>
                <td class="thisTD">
                    ...
                    @php
                        $adjustedBalance = floatval($supplier_k->prev_balance);
                    @endphp
                    
                    @if(isset($bondSuppliers) && !$bondSuppliers->isEmpty())
                        @foreach ($bondSuppliers as $key => $bond)
                            @php
                                if ($bond->type == 'قبض') {
                                    $adjustedBalance -= floatval($bond->amount); // قبض decreases balance
                                } elseif ($bond->type == 'صرف') {
                                    $adjustedBalance += floatval($bond->amount); // صرف increases balance
                                }
                            @endphp
                        @endforeach
                    @endif
                            ...
                            @else
                                @if (isset($supplier_k) && !empty($supplier_k))
                                    @if (!empty($supplier_k->supplier_email))
                                        <form class="d-inline" action="{{ route('supplier.summary.send') }}"
                                            method="post">
                                            @csrf
                                            @method('POST')
